...
//un string dinamicp es un string al cual le podemos cambiar su tamaño
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(){
	...
	JDT.nombre=(char*)malloc((strlen(aux)+1)*sizeof(char));//Con esto resevamos el espacio necesario
	//al final es (+1) debido a que no cuenta el espacio con '\0'
	if(JDT.nombre==NULL){
		printf("No sea ha podido reservar memoria.\n");
		exit(1);//esto nos ahorra hacer muchos ifelse{ }
		}
	}
...
//------------------------------------------------------------------------------
	...
	//siempre liberar la memoria al final
	//lo que hacemos es eliminar los valores de la memoria asignada
	free(JDT.nombre);
	free(JDT.autor);
	return 0;
}
